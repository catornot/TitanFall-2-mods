...
var function OnWeaponPrimaryAttack_holopilot( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	{
		if ( GetEntArrayByScriptName( player.GetPlayerName() + "_decoy" ).len() != 0 )
			return

		foreach ( entity d in GetEntArrayByScriptName( player.GetPlayerName() + "_decoy" ) )
			d.Die()

		// player.StopRecordingAnimation() // crash
		player.Signal( "StopAllRecordings" )

		numberOfDecoysToMake = 1
		...
		for ( int i = 0; i < numberOfDecoysToMake; ++i )
		{
			...
			// 	decoy.GiveWeapon( weapons[0].GetWeaponClassName() )
			// 	// weapon.SetMods( weapons[0].GetMods() )
			// }
			...
			thread KillAfterTimeout( DECOY_DURATION, decoy )
			decoy.kv.VisibilityFlags = ENTITY_VISIBLE_TO_EVERYONE
			// decoy.SetFootstepType( "holopath" )
			...
			thread MimicOwner( player, decoy )
	...
	vector function CalculateAngleSegmentForDecoy( int loopIteration, vector angleSegment )
	{
		...
		void function Decoy_FlagFX( entity decoy, entity decoyChildEnt )
		{
			...
			entity flagTrailFX = StartParticleEffectOnEntity_ReturnEntity(
				decoyChildEnt,
				GetParticleSystemIndex( FLAG_FX_ENEMY ),
				FX_PATTACH_POINT_FOLLOW,
				decoyChildEnt.LookupAttachment( "fx_end" )
			)
	...
	void function KillAfterTimeout( float time, entity decoy )
	{
		EndSignal( decoy, "OnDeath" )
		EndSignal( decoy, "OnDestroy" )
