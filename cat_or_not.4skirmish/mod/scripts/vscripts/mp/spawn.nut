...
entity function GetBestSpawnpoint( entity player, array<entity> spawnpoints )
{
	...
	if ( validSpawns.len() == 0 )
	{
		entity start = CreateScriptRef( < 0, 0, 1000 > )
		start.kv.editorclass = "info_player_start"
		start.SetScriptName( "info_player_start" )
		start.s.lastUsedTime <- -999
	}

	return validSpawns[
		RandomInt( validSpawns.len() )
	] // slightly randomize it
}
...
void function RateSpawnpoints_Generic( int checkClass, array<entity> spawnpoints, int team, entity player )
{
	...
	// perhaps dead players could be used to calculate some sort of activity rating? so
	// high-activity points with an even balance of friendly/unfriendly players are preferred
...
entity function DecideSpawnZone_Generic( array<entity> spawnzones, int team )
{
	...
	if ( needNewZone )
	{
		...
		possibleZones.sort(
			int function( entity a, entity b )
			{
