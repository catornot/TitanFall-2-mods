...
{
    SpawnBliskKillShop( <1170, -1175, -15>, <0, 45, 0> )
    if ( !GetDisabledElements().contains( "random_teleport" ) )
    {
    ...
    CreateSimpleButton( <4516, -3206, 217>, <0,0,0>, "to summon sarah briggs", callback_SarahBriggsButtonTriggered )
...
void function SpawnBliskKillShop( vector origin, vector angles ) {
    ...
    prop.SetUsePrompts( "Hold %use% to get a target", "Press %use% to get a target" )
...
void function GetNewTarget( entity player )
{
    ...
    {
        NSSendPopUpMessageToPlayer( player, "No Targets :skull:" )
        return
    }
	...
	function() : ( player, target, unique_id, other_players  )
		{
			...
			{
				NSSendPopUpMessageToPlayer( player, "You failed to kill the target" )
			}
            else 
            {
                NSSendPopUpMessageToPlayer( player, "Great Job!" )
                StimPlayer( player, 5 )
            }
		}
	)
...
void function SpawnRacingPilot( vector origin, vector angles, CameraPath path ) {
    ...
    pilot.SetUsePrompts( "Hold %use% to start a race", "Press %use% to start a race" )
...
void function HandleRaceIntroCamera( entity player, CameraPath path  )
{
    ...
    NSSendInfoMessageToPlayer( player, "Crouch to stop the intro." )
...
{
    CreateSimpleButton( origin, <0, 90, 0>, "to randomly teleport", callback_RandTelportButtonTriggered )
}
...
void function callback_SarahBriggsButtonTriggered( entity panel, entity player )
{
    ...
    thread SarahDefenseThink( player.GetTeam() )
}
...
void function SarahDefenseThink( int team )
{
    ...
    OnThreadEnd(
		function() : ( ship, sarah )
		{
            ...
            foreach ( entity player in GetPlayerArray() )
                NSSendInfoMessageToPlayer( player, "Sarah Died :(" )
		}
	)
    ...
    foreach ( entity player in GetPlayerArray() )
    {
        if ( player.GetTeam() == team )
            NSSendInfoMessageToPlayer( player, "Defend Sarah's ship from the enemies" )
        else
            NSSendInfoMessageToPlayer( player, "Consider making an attack on Sarah's ship" )
    }
    
    PhaseShift( sarah, 0 , 0.1 )
    GoblinRiderAnimate( sarah, model, 0, "RESCUE" )
    TakeAllWeapons( sarah )
    sarah.GiveWeapon( "mp_weapon_arc_launcher" )
    ...
    entity newtitan = CreateNPCTitan( "titan_stryder", team, sarah.GetOrigin(), <0,0,0>, [] )
    SetSpawnOption_NPCTitan( newtitan, TITAN_HENCH )
    SetSpawnOption_AISettings( newtitan, "npc_titan_stryder_rocketeer_dash_core" )
    DispatchSpawn( newtitan )
    newtitan.SetMaxHealth( newtitan.GetMaxHealth() * 4 );
    ...
    OnThreadEnd(
		function() : ( newtitan, sarahShip )
		{
            if ( IsAlive( newtitan ) )
            {
                ...
                foreach ( entity player in GetPlayerArray() )
                    NSSendInfoMessageToPlayer( player, "Sarah traveled around homestead successfully because LORE" )
            }
            else {
                foreach ( entity player in GetPlayerArray() )
                    NSSendInfoMessageToPlayer( player, "Sarah Died :(" )
    ...
    {
        if ( player.GetTeam() == team )
            NSSendInfoMessageToPlayer( player, "Defend Sarah for 120 seconds" )
    }
    ...
    for(;;)
    {
        ...
        switch ( count )
        {
            case 60:
                ...
                {
                    if ( player.GetTeam() == team )
                        NSSendInfoMessageToPlayer( player, "60 seconds left" )
                }
                break
            case 90:
                foreach ( entity player in GetPlayerArray() )
                {
                    if ( player.GetTeam() == team )
                        NSSendInfoMessageToPlayer( player, "30 seconds left" )
                }
                break
            case 110:
                foreach ( entity player in GetPlayerArray() )
                {
                    if ( player.GetTeam() == team )
                        NSSendInfoMessageToPlayer( player, "10 seconds left" )
                }
                break
