...
void function HackingOperation( entity player )
{
    ...
    OnThreadEnd(
		function() : ( player, ship, ent_cleanup )
		{
            ...
            if ( file.mission_started && IsValid( player ) && !IsAlive( player ) )
                Chat_ServerPrivateMessage( player, "You failed to the mission", false, false )
    ...
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m Get on the ship", false, false )
    ...
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m We are giving you a drone.", false, false )
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m Go North towards the tower and up the hill.", false, false )
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m You will find a terminal to hack.", false, false )
    ...
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m Defend the terminal", false, false )
    ...
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m Good job!", false, false )

    ShipFlyToPos( ship, CLVec( <8283, 9000, 2991> ), <0,-10,0> )
    WaitSignal( ship, "Goal" )
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m We need to leave now", false, false )
    ...
    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m You can Go now.", false )
    player.ClearParent()
    waitthread WaitForPlayerToJumpOff( model, player )

    Chat_ServerPrivateMessage( player, "\x1b[32m[Sarah]\x1b[0m Good Bye", false, false )
