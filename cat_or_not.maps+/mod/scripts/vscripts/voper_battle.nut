...
void function StartVoperBattle( int varient )
{
	...
	link.NonPhysicsRotateTo( < 0, 180, 0 >, 1.0e-7, 0, 0 )
	...
	file.viperShip.mover.NonPhysicsRotateTo( < 0, 180, 0 >, 1.0e-7, 0, 0 )

	if ( varient == 0 )
	{
		file.RespawnShip =
			CreateExpensiveScriptMoverModel( $"models/vehicle/goblin_dropship/goblin_dropship.mdl", Player_SpawnPoint_Voper + < 0, 0, 1500 >, < 0, 90, 0 >, 6, 100000 )
...
void function StartIntro( ShipStruct viperShip, int varient )
{
	...
	link.NonPhysicsRotateTo( < 0, 180, 0 >, 1.0e-8, 0, 0 )
