...
global function CreateLightSpriteCustom

void function precache_needed_stuff_like_rn()
{
	PrecacheModel( $"models/robots/mobile_hardpoint/mobile_hardpoint.mdl" )
	PrecacheModel( $"models/robots/mobile_hardpoint_laptop/mobile_hardpoint_laptop.mdl" )
    ...
    // PrecacheModel( $"models/industrial/tripod_cone_v1_low_on.mdl" )
    PrecacheModel( $"models/ball/sphere_conv.mdl" )
    PrecacheModel( $"models/rocks/rock_small_02.mdl" )
}
...
void function CatPrint( string text )
{
    ...
    // dialog.message = "testing, yes gaming \ngame"
    // dialog.showPCBackButton = true
    // dialog.messageColor = [161, 0, 255, 255]

    // NSAddDialogButton( dialog, "whar", void function ( entity player ) { printt("he did it", player ) } )

    // for(;;)
    // {
        // NSSendServerDialog( GetPlayerArray()[0], dialog )
    //     wait 0
    // }
}
...
entity function CreateHackPanel( vector origin, vector angles, void functionref( entity, entity ) panelHacked )
{
	...
	SetControlPanelUseFunc( panel, function( panel, player ) : ( panelHacked )
    {
        expect entity( panel )
        expect entity( player )
        panelHacked( panel, player )
    } )
