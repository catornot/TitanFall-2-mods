	...
	while ( IsValid( CAR.car ) && CAR.health > 0.0 && IsValid( CAR.turret ) )
	{
		...
		if ( CAR.hasPlayer && !CAR.IsFalling )
		{
			...
			if ( keys[ KF ] && keys[ KL ] && canMoveF )
			{
				...
				CAR.offsetlocation =
					_PositionBasedOnAngle( originalOffsetPoint + CAR.car.GetOrigin(), -CAR.car.GetAngles().y - 10.0, CAR.car.GetOrigin() ) - CAR.car.GetOrigin()
...
bool function CanMoveB( CARstruct CAR )
{
	...
	foreach ( float angle in [ 10.0, 0.0, -10.0 ] )
	{
		...
		traceResult =
			TraceLine(
				CAR.CARmover.GetOrigin() + rayOffset,
				rayOffset + CAR.CARmover.GetOrigin() + NewRayOffset * -4,
				[ CAR.car, CAR.turret, CAR.CARmover ],
				TRACE_MASK_SHOT,
...
bool function IsFalling( CARstruct CAR )
{
	...
	foreach ( vector wheeloffset in wheeloffsets )
	{
		...
		traceResult =
			TraceLine(
				CAR.CARmover.GetOrigin() + NewWheeloffset,
				CAR.CARmover.GetOrigin() + NewWheeloffset - < 0, 0, 50 >,
				[ CAR.car, CAR.CARmover ],
				TRACE_MASK_SHOT,
				TRACE_COLLISION_GROUP_NONE
			)
...
bool function TryRise( CARstruct CAR, vector position )
{
	...
	foreach ( float angle in [ 10.0, 0.0, -10.0 ] )
	{
		traceResult =
			TraceHull(
				position + rayClimbOffset2,
				rayClimbOffset1 + position,
				rayClimbMin,
				rayClimbMax,
				[ CAR.car, CAR.CARmover, CAR.turret ],
				TRACE_MASK_SHOT,
				TRACE_COLLISION_GROUP_NONE
			)
